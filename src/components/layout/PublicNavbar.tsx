...
export default function PublicNavbar() {
  ...
  const showLogin = authStatus?.login_enabled;
  const showSignup = authStatus?.signup_enabled;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {dashboardLink ? (
              <Link to={dashboardLink} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
                Dashboard
              </Link>
            ) : (
              <>
            ...
            <div className="p-4">
              ...
              <div className="mt-12 space-y-1">
                ...
                <div className="space-y-3">
                  {dashboardLink ? (
                    <Link to={dashboardLink} onClick={() => setIsMenuOpen(false)} className="block px-3 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-center font-medium">Dashboard</Link>
                  ) : (
                    <>
                      {showLogin && (
