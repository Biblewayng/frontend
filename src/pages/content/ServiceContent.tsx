...
export default function ServiceContent() {
  const { serviceTimes: services, serviceTimesLoading: loading, createServiceTime, updateServiceTime, deleteServiceTime } = useContent();
  const [saving, setSaving] = useState(false);
  const [updating, setUpdating] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [showAddService, setShowAddService] = useState(false);
  const [editingService, setEditingService] = useState<ServiceTime | null>(null);
  const [deleteConfirm, setDeleteConfirm] = useState<ServiceTime | null>(null);
    ...
    <div>
      ...
      <div className="space-y-4">
        {services.length === 0 ? (
          ...
          services.map((service: ServiceTime) => (
          <div key={service.id} className="flex flex-col sm:flex-row items-start justify-between p-4 border border-gray-200 rounded-lg gap-3">
            <div className="flex-1 min-w-0">
              <p className="font-medium text-gray-900 break-words">{service.service || service.service_type}</p>
            ...
            <div className="relative bg-white rounded-lg px-4 pt-5 pb-4 shadow-xl w-full max-w-lg sm:p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={editingService.service || editingService.service_type || ''}
