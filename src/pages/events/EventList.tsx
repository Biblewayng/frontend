...
export default function EventList() {
  const { events, loading, page, totalPages, setPage } = useEvents();
  ...
  const selectedEventDetails = selectedEvent
    ? events.find((e: Event) => e.id === selectedEvent)
    : null;
              ...
              <div className="space-y-4">
                {selectedEventDetails && (
                  <>
                    ...
                    <p className="text-gray-600">{selectedEventDetails.description}</p>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div><strong>Date:</strong> {new Date(selectedEventDetails.date || '').toLocaleDateString()}</div>
                      <div><strong>Time:</strong> {selectedEventDetails.time}</div>
