...
export default function FormsList({ filterStatus }: FormsListProps) {
  const { forms, loading, deleteForm, deleteForms, page, totalPages, setPage } = useForms(filterStatus !== 'all' ? { status: filterStatus } : undefined);
  ...
  const handleShare = (id: string) => {
    ...
    if (form?.status !== 'active') {
      toast.error('Only active forms can be shared');
      return;
    }
    setSelectedForm(id);
  ...
  const handleExport = async (format: 'csv' | 'pdf') => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/forms/export`, {
        method: 'POST',
      ...
      await downloadFile(url, `forms-export-${Date.now()}.${format}`);
  ...
  const filteredForms: Form[] = forms;
  ...
  const toggleAll = () => {
    if (selectedForms.length === filteredForms.length) {
      ...
      setSelectedForms(filteredForms.map((f: Form) => f.id));
    }
  };
