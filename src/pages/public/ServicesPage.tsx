...
export default function ServicesPage() {
  ...
  const serviceTimes: ServiceTime[] = Array.isArray(data) ? data : (data as any)?.data || [];
          ...
          {loading ? (
            ...
            <div className={`grid gap-8 ${serviceTimes.length === 1 ? 'grid-cols-1 max-w-md mx-auto' : serviceTimes.length === 2 ? 'grid-cols-1 md:grid-cols-2 max-w-4xl mx-auto' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'}`}>
              {serviceTimes.map((service, index) => {
                ...
                return (
                  <div key={service.id ?? index} className="flex items-start space-x-4 p-8 bg-white rounded-lg shadow-md border border-gray-200 min-h-[200px] transition-all duration-500 ease-in-out hover:shadow-xl hover:-translate-y-2 hover:border-blue-300 cursor-pointer">
                    ...
                    <div>
                      ...
                      <p className={`text-sm ${color.text} font-medium`}>{service.day} • {service.time ? formatTime(service.time) : ''}</p>
                    </div>
                  </div>
                );
              })}
